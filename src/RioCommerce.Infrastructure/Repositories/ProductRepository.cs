    ...
    public async Task<PagedResult<ProductListItem>> GetFilteredAsync(ProductFilterRequest filter)
    {
        ...
        var items = await ordered.Skip((filter.Page - 1) * filter.PageSize).Take(filter.PageSize)
            .Select(p => new ProductListItem
            {
                ...
                SpecialPrice = p.SpecialPrice, SpecialPriceStartDateUtc = p.SpecialPriceStartDateUtc, SpecialPriceEndDateUtc = p.SpecialPriceEndDateUtc, SchoolStudentPrice = p.SchoolStudentPrice,
                Badge = p.Badge,
